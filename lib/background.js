...
browser.omnibox.onInputChanged.addListener(async (input, suggest) => {
  try {
    const {noFullSearchResultsLink} =
      await browser.storage.local.get('noFullSearchResultsLink');
    const numSuggestions = noFullSearchResultsLink ? 5 : 4;

    // Note: `activeTab` permission is not enough (only
    //   *accepting* an omnibox suggestion will allow this use);
    //   if it could work, use "https://www.google.com/*" in place
    //   of `<all_urls>`
    let currentTab = await browser.tabs.query({
      currentWindow: true,
      active: true,
      url: '*://*/*' // Restricts to http/s, ws (not chrome:// URLs)
    });
    currentTab = currentTab && currentTab[0];

    // console.log('currentTab', currentTab && currentTab.url);
    const domain = currentTab && currentTab.url
      ? new URL(currentTab.url).host
      : null;
    const searchURL = `https://www.google.com/search?q=${input}+${domain ? 'site%3A' + domain : ''}&ie=utf-8&oe=utf-8`;
    ...
    */
    ...
    links.some((link) => {
      ...
      link = link.parentNode.matches('a[href]')
        ? link.parentNode
        : link.querySelector('a[href]');
      if (!link || link.style.display === 'none') {
        return false;
      }
      // Doing `link.href` (at least in Firefox) gets us a path
      //   relative to the extension which we don't want
      const url = new URL(
        link.getAttribute('href'), searchURL
      ).href;
      ...
      results.push({
        ...
        // Was showing URL and title, but as Firefox was showing
        //   cursor at end of line, one couldn't see the URL
        // This value will show when highlighted within list
        // content: _('result_content', [url, title,
        //   description]),

        content: url,
        // Show title and URL
        description: escapeXML(
          _('result_description', [url, title])
        ) // This is what shows for each list item
      });
      return false;
    });
